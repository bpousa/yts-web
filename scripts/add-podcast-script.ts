/**
 * One-time script to add the Cosmic Origin podcast script to the database
 * Run with: npx ts-node scripts/add-podcast-script.ts
 */
...
const PODCAST_SCRIPT = `# PODCAST SCRIPT: THE COSMIC ORIGIN
...
async function main() {
  const userEmail = 'mike@appendment.com'
  ...
  if (userError) {
    ...
    const user = authUsers?.find(u => u.email === userEmail)
    if (!user) {
    ...
    const { data: transcript, error: insertError } = await supabase
      ...
      .insert({
        user_id: user.id,
        project_id: null,
        video_id: `custom-${Date.now()}`,
        video_title: 'The Cosmic Origin - Podcast Script',
        video_url: null,
    ...
    console.log('Title:', transcript.video_title)
    return
  }
}
